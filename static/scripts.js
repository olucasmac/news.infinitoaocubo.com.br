...
// Decodifica as entidades HTML nos itens do feed
function decodeFeedItems() {
    var elements = document.querySelectorAll(".feed-item .content h2, .feed-item .content p");
    elements.forEach(function(element) {
        element.innerHTML = decodeHTMLEntities(element.innerHTML);
    });
}

// Função para filtrar os feeds
...
// Adiciona a barra de pesquisa se o parâmetro 'search' estiver presente na URL
function addSearchBar() {
    ...
    if (showSearch) {
        ...
        searchContainer.innerHTML = '<input type="text" id="searchInput" placeholder="Search for articles...">';
        const filters = document.getElementById('filters');
        filters.insertAdjacentElement('afterend', searchContainer);

        // Evento de entrada na barra de pesquisa
        const searchInput = document.getElementById('searchInput');
        searchInput.addEventListener('input', filterFeeds);
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    addSearchBar(); // Adicionar a barra de pesquisa se o parâmetro estiver presente

    const urlParams = new URLSearchParams(window.location.search);
    showButtons = urlParams.get('buttons') === 'true';
...
function renderFeedItems(data, page) {
    ...
    decodeFeedItems(); // Decodificar itens do feed após renderizá-los
}
